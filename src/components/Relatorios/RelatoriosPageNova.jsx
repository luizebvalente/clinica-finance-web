...
import RelatorioTendencias from './RelatorioTendencias';
import RelatorioPorCategoria from './RelatorioPorCategoria';

const RelatoriosPageNova = () => {
  ...
  const abas = [
    ...
    { id: 'tendencias', label: 'Tendências', icone: TrendingUp },
    { id: 'categorias', label: 'Por Categoria', icone: PieChart },
    { id: 'relatorios', label: 'Relatórios Detalhados', icone: FileSpreadsheet }
  ];
          ...
          {abaSelecionada === 'resumo' && (
            <div className="space-y-6">
              <div className="text-center py-8">
                ...
                <p className="text-gray-600">Visão geral dos principais indicadores financeiros e operacionais</p>
              </div>
            </div>
          )}

          {abaSelecionada === 'comparativo' && (
            <RelatorioComparativo filtros={filtros} />
          )}

          {abaSelecionada === 'tendencias' && (
            <RelatorioTendencias filtros={filtros} />
          )}

          {abaSelecionada === 'categorias' && (
            <div className="space-y-6">
              <RelatorioPorCategoria tipo="receitas" filtros={filtros} />
              <RelatorioPorCategoria tipo="despesas" filtros={filtros} />
            </div>
          )}
...
export default RelatoriosPageNova;
